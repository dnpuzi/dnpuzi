  ...
  <div class="wrapper">
    ...
    <Form :model="devForm" :rules="rules" ref="devForm">
      ...
      <FormItem label-width="100px">
        <Button type="primary" @click="handleDevSubmit('devForm')" v-show="!isAdded">立即创建</Button>
        <Button type="warning" v-show="isAdded" @click="handleDevModify">修改(此功能待开发)</Button>
        <Button @click="reset('devForm')" v-show="!isAdded">重置</Button>
      </FormItem>
    </Form>
    <div class="attach_info_wrapper" v-show="attachInfo">
  ...
  export default {
    ...
    mounted() {
      if (this.$route.query.catid) {
        this.devForm.cat_id = this.$route.query.catid;
      }
      document.title = '添加电脑';
